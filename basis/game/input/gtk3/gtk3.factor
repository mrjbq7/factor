...
! Copyright (C) 2024 Factor contributors - Ported to GTK3
! See https://factorcode.org/license.txt for BSD license.
USING: accessors alien.c-types alien.data alien.syntax arrays
assocs bit-arrays destructors game.input gdk3.ffi
...
M: gtk3-game-input-backend (open-game-input)
    reset-mouse ;

M: gtk3-game-input-backend (close-game-input)
    ;

M: gtk3-game-input-backend (reset-game-input)
    ;

M: gtk3-game-input-backend get-controllers
    get-input-events-joysticks values [ <linux-controller> ] map ;

M: gtk3-game-input-backend product-string
    meta>> "name" of ;

M: gtk3-game-input-backend product-id
    meta>> "id" of ;

M: gtk3-game-input-backend instance-id
    drop f ;

M: gtk3-game-input-backend read-controller
...
M: gtk3-game-input-backend calibrate-controller
    drop ;

M: gtk3-game-input-backend vibrate-controller
    3drop ;
...
M: gtk3-game-input-backend read-keyboard
...
M: gtk3-game-input-backend read-mouse
    query-pointer
    mouse-state new
    swap 256 /i >>buttons
    swap 400 - >>dy
...
M: gtk3-game-input-backend reset-mouse
